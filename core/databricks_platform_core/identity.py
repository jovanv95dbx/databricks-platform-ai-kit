...
and any freeform tags defined in ``.databricks-platform-kit.yaml``.

Example user-agent string::

    databricks-platform-kit/0.1.0 databricks-sdk-py/0.73.0 python/3.11.13 os/darwin auth/pat project/my-repo

Example config file (``.databricks-platform-kit.yaml`` at repo root)::
...
PRODUCT_NAME = "databricks-platform-kit"

DESCRIPTION_FOOTER = "Built with Databricks Platform Kit"
...
_CONFIG_FILENAME = ".databricks-platform-kit.yaml"
...
def _load_config() -> Dict[str, Any]:
    """Load ``.databricks-platform-kit.yaml`` from the git repo root or cwd.
...
def detect_project_name() -> str:
    ...
    Detection priority:
        1. ``project`` field in ``.databricks-platform-kit.yaml``
...
def get_default_tags() -> Dict[str, str]:
    ...
    Merges:
        1. ``created_by: databricks-platform-kit`` (always)
        2. ``project: <detected or configured>`` (always)
        3. Freeform tags from ``.databricks-platform-kit.yaml`` ``tags:`` section
